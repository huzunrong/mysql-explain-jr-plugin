...
import com.example.plugin.mysql.common.Constant;
import com.example.plugin.mysql.util.StringUtils;
...
import static com.example.plugin.mysql.common.Config.extraOptimizationItems;
import static com.example.plugin.mysql.common.Config.filterSqlKeywords;
import static com.example.plugin.mysql.common.Config.showSQL;
import static com.example.plugin.mysql.common.Config.typeOptimizationItems;
...
public final class MySQLExplain {
    ...
    private static boolean processBefore(String sql) {
        ...
        if (StringUtils.isEmptyArray(s)) {
            logger.warnEcho("不支持的EXPLAIN: sql.split(\" \", 2) empty");
            return false;
        }
        ...
        if (!Constant.SUPPORTED_EXPLAIN_SQL.contains(s[0])) {
            if (!s[0].startsWith("SELECT*") && !s[0].startsWith("select*")) {
                logger.warnEcho("不支持的EXPLAIN - 不支持的语句: {}, {}", s[0], Constant.SUPPORTED_EXPLAIN_SQL);
                return false;
            }
        }
